...
# ------------------------------------------------------------------
# The prompt is read from system_prompt.txt in your repo root.
# Just edit that .txt file to change the AI's behavior -- no code changes.
# It runs server-side, so the browser never sees it.
...
def chat()
    try
        ...
        completion = client.chat.completions.create(
            model=nvidianemotron-3-super-120b-a12b,
            messages=messages,
            temperature=1,
            top_p=0.95,
            max_tokens=4096,
